...
class PPO3(ActorCriticRLModel):
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, values, neglogpacs, update,
        ...
        # Break and save on approxkl being too large
        if approxkl > 0.1:
            self.save("ppo2_mario(earlystop)")
            print(f'Force stop on {self.n_steps} due to approxkl being too large')
            return
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                # if self.num_timesteps % 61440 == 0:
                #     self.save(f"ppo2_mario{self.num_timesteps}")
                if self.count % 10 == 0:
                    self.save(f"ppo2_mario{self.count}")
                self.count += 1
